...
def custom_round(bound, awayFrom = None): # awayFrom is the value that the bound param must be greater than EPSILON away from (can be set to previous counterexample, or segment bounds)
        ...
        while stack:
            ...
            if exitCode == "sat":
                if set_not_split:
                    ...
                    prev_curr_counterex_delta_signs = [] # ALIGNED EDGE CASE # this is which direction prev counterex is located relative to curr counter ex (e.g. [-1, -1], [-1, 0], [-1, 1], [0, 1], etc.)
                    for key in inputVars:
                        value = custom_round(vals[key], awayFrom = prev_cntr_ex[key])
                        split_idxs.append(value)

                        diff = prev_cntr_ex[key] - value
                        prev_counterex_relative_segment.append(int(diff > 0))

                        delta_signs = int(diff / abs(diff)) if diff != 0 else 0 # ALIGNED EDGE CASE
                        prev_curr_counterex_delta_signs.append(delta_signs) # ALIGNED EDGE CASE
                    isAlignedCase = (len(prev_curr_counterex_delta_signs) - prev_curr_counterex_delta_signs.count(0)) == 1 # ALIGNED EDGE CASE
                    ...
                    for segment in itertools.product(*[[0, 1] for idx in range(len(inputVars))]): # For example, in 2D input case, (0, 0) is bottom-left quadrant, (0, 1) is top-left, (1, 0) is bottom-right, and (1, 1) is top-right
                        bottom_left = []
                        top_right = []
                        alignedCasePrevCounterExSegment = [] # ALIGNED EDGE CASE
                        skipSegment = False
                        ...
                        for dim_idx, dim_val in enumerate(segment): # Note: next_segment_offset only affects particular dimensions of different bounds of different segments
                            if (isAlignedCase): alignedCasePrevCounterExSegment.append(min(prev_curr_counterex_delta_signs[dim_idx] + 1, 1)) # ALIGNED EDGE CASE
                        ...
                        usePrevCounterExOutput = (list(segment) == alignedCasePrevCounterExSegment) if isAlignedCase else (list(segment) == prev_counterex_relative_segment) # ALIGNED EDGE CASE
