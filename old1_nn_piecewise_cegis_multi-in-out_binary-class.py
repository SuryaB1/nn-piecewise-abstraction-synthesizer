...
def main():
    ...
    for i in tqdm(range(len(outputVars))):
        ...
        while stack:
            ...
            options = Marabou.createOptions(snc=True)
            ...
            if exitCode == "sat":
                if set_not_split:
                    ...
                    split_idxs = []
                    prev_curr_counterex_delta_signs = [] # which direction prev counterex is located relative to curr counter ex (e.g. [-1, -1], [-1, 0], [-1, 1], [0, 1], etc.)
                    ...
                    for corner_idx_comb in itertools.product(*[[0, 1] for idx in range(len(inputVars))]): # For example, in 2D input case, (0, 0) is bottom-left quadrant, (0, 1) is top-left, (1, 0) is bottom-right, and (1, 1) is top-right
                        bottom_left = []
                        top_right = []
                        isNotPrevCounterExSegment = list(corner_idx_comb) != prev_counterex_relative_segment
                        skipSegment = False

                        # Get upper bound of split's lower segment and lower bound of upper segment
                        for dim_idx, dim_val in enumerate(corner_idx_comb): # Note: next_segment_offset only affects particular dimensions of different bounds of different segments
                            # if (DEBUG): print("prev_counterex_relative_segment[dim_idx] == dim_val:", prev_counterex_relative_segment[dim_idx] == dim_val)
                            ...
                            curr_dim_offset = EPSILON * int( ((dim_val == prev_counterex_relative_segment[dim_idx]) and not isAlignedCase) # offset (gap) only when current dim of current segment's lower bound is same as corresponding dim of lower bound of segment containing prev counter_ex
                                                            or ( (dim_val == min(prev_curr_counterex_delta_signs[dim_idx] + 1, 1)) and isAlignedCase) ) # in the aligned counterex case, the segment that is offsetted on all dims is the delta-sign representation of where the prev ctrex is + 1 on all dims (maxed at 1)
                        ...
                        # if (DEBUG): print("corner_idx_comb, prev_counterex_relative_segment:", list(corner_idx_comb), prev_counterex_relative_segment, int( (prev_cntr_ex[outputVarIdx] > 0) ^ (list(corner_idx_comb) != prev_counterex_relative_segment) ))
                        val = int( (prev_cntr_ex[outputVarIdx] > 0) ^ isNotPrevCounterExSegment) # or (set equal to left or above quadrant based on if two other ones or on edge)
                        new_segment = [bottom_left, top_right, val]
                        if (DEBUG): print(f"a split segment: {new_segment}")
                        stack.append(new_segment) # Add split segment to stack
